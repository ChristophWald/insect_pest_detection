...
#some flags
only_evaluation = False #if True, only rectangles (single file) and metrics are saved
inspection = False #if True, saves images of single steps in a test folder
process_labels = True #if True, also crops the given labels

###Setup
image_folder = "/user/christoph.wald/u15287/big-scratch/only_thrips/train_labeled/images_uncropped"
label_folder = "/user/christoph.wald/u15287/big-scratch/only_thrips/train_labeled/labels_uncropped"

output_folder_images_masked = "/user/christoph.wald/u15287/big-scratch/only_thrips/train_labeled/images_masked"
output_folder_images_cropped = "/user/christoph.wald/u15287/big-scratch/only_thrips/train_labeled/images_cropped"
output_folder_labels = "/user/christoph.wald/u15287/big-scratch/only_thrips/train_labeled/labels_cropped"
...
for i, image_file in enumerate(image_files):
    ...
    if not only_evaluation: cv2.imwrite(os.path.join(output_folder_images_cropped, image_file), cropped_image)
    ...
    if not only_evaluation: cv2.imwrite(os.path.join(output_folder_images_masked, image_file), cropped_image_wo_grid)
    
    #finding the rectangles given by the yolo labels
    if not only_evaluation and process_labels:
