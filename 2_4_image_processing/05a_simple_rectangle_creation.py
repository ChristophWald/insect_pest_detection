...
for species in ["BRAIIM", "LIRIBO", "TRIAVA"]:
    print(species)
    image_files = sorted(os.listdir(image_folder))
    #change to run only on one insect pest class
    image_files = [f for f in image_files if species in f]
    ...
    for i, image_file in enumerate(image_files):
        ...
        rectangles, v = get_list_of_rectangles(image, min_area_contour, max_area_contour, scale = 1.5, max_ratio = 2)
    ...
    with open(f"rectangles_{species}.txt", "w") as f:
    ...
    if process_labels:
        ...
        grouped.to_csv("metrics_test_delete.csv")
