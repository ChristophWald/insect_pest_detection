...
def copy_pair(files, dest_img_dir, dest_lbl_dir):
    ...
    '''
    for f in files:
        src_img = os.path.join(os.path.join(input_dir, "04_images_cropped"), f)
        src_lbl = os.path.join(os.path.join(input_dir, "05_created_labels"), f.replace('.jpg', '.txt'))
...
input_dir = "/user/christoph.wald/u15287/big-scratch/02_splitted_data/train_labeled/SSL"
output_dir = "/user/christoph.wald/u15287/big-scratch/02_splitted_data/train_labeled/SSL/split"
...
image_files = [f for f in os.listdir(os.path.join(input_dir, "04_images_cropped")) if f.endswith(('.jpg', '.png'))]
separated = []
