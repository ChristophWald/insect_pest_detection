...
def tile_and_save_images_only(image_path, dest_path,
                              tile_size=640, stride=440, pad_value=(114,114,114)):
    """
    Tile a single image into smaller patches and save them.
    No labels are required; empty label files will be created.
    """
    import cv2
    import os
    import math
    ...
    image = cv2.imread(image_path)
    padded_img = pad_to_multiple(image, tile_size)
    p_h, p_w = padded_img.shape[:2]
    ...
    for y in range(0, p_h - tile_size + 1, stride):
        for x in range(0, p_w - tile_size + 1, stride):
            ...
            label_filename = f"{os.path.splitext(os.path.basename(image_path))[0]}_tile_{tile_id}.txt"
            label_path_out = os.path.join(labels_out, label_filename)
            open(label_path_out, 'w').close()  # creates empty file
...
for split, img_path in img_paths.items():
    ...
    dest_path = os.path.join(base_path, "tiles_1280", split)
    os.makedirs(dest_path, exist_ok=True)
