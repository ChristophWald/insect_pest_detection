...
import math
import numpy as np
from modules import load_yolo_labels, compute_intersection_area, pad_to_multiple
import ast
...
def tile_and_save(image_path, 
                  label_path, 
                  dest_path, 
                  tile_size=640, 
                  stride=440, 
                  min_inside_ratio=0.8, #only labels that lie at least [min_inside_ratio] inside the tile are kept
                  yolo=True, #if True yolo-format labels are expected, if False absolute coordinates with xywh-format
                  save_only_with_labels=False #if True, tiles without labels (empty tiles) are not saved 
                  ):
    ...
    for y in range(0, p_h - tile_size + 1, stride):
        for x in range(0, p_w - tile_size + 1, stride):
            ...
            # Skip tiles without labels if flag is set
            if save_only_with_labels and len(tile_labels) == 0:
                continue
...
img_path = "/user/christoph.wald/u15287/big-scratch/02_splitted_data/train_labeled/SSL/03_images_masked"
label_path = "/user/christoph.wald/u15287/big-scratch/02_splitted_data/train_labeled/SSL/05_created_labels"
dest_path = "/user/christoph.wald/u15287/big-scratch/02_splitted_data/train_labeled/SSL/tiles_mininside08_masked"
os.makedirs(dest_path, exist_ok=True)
...
dest_path = "/user/christoph.wald/u15287/big-scratch/02_splitted_data/augmented_data/tiles_1280"
os.makedirs(dest_path, exist_ok=True)
...
label_path = "/user/christoph.wald/u15287/big-scratch/02_splitted_data/train_labeled/SSL/05_created_labels"
dest_path = "/user/christoph.wald/u15287/big-scratch/02_splitted_data/train_labeled/SSL/tiles_masked_augmented_1280"
os.makedirs(dest_path, exist_ok=True)
files = os.listdir(img_path)
...
for file in files:
    ...
    tile_and_save(img, label, dest_path, 
                  tile_size=1280, stride= 1080,
                  yolo=False, save_only_with_labels=True)
