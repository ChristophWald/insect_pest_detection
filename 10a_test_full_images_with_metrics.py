...
def compare_labels(
    ...
    # Per-class metrics
    per_class_result = {}
    for cls in sorted(set(tp.keys()) | set(fp.keys()) | set(fn.keys())):
        cls = int(cls)
        tpi = tp[cls]
        fpi = fp[cls]
        fni = fn[cls]
...
final_per_class = defaultdict(lambda: {"TP": 0, "FP": 0, "FN": 0})

for res in results:
    summary = res["summary"]
    final_summary["TP"] += summary["TP"]
    final_summary["FP"] += summary["FP"]
    final_summary["FN"] += summary["FN"]

    for cls, cls_data in res["per_class"].items():
        final_per_class[cls]["TP"] += cls_data["TP"]
        final_per_class[cls]["FP"] += cls_data["FP"]
        final_per_class[cls]["FN"] += cls_data["FN"]

# Compute precision and recall
final_summary["precision"] = (
    final_summary["TP"] / (final_summary["TP"] + final_summary["FP"])
    if (final_summary["TP"] + final_summary["FP"]) > 0 else 0.0
)
final_summary["recall"] = (
    final_summary["TP"] / (final_summary["TP"] + final_summary["FN"])
    if (final_summary["TP"] + final_summary["FN"]) > 0 else 0.0
)

# Compute per-class metrics
for cls in final_per_class:
    tp = final_per_class[cls]["TP"]
    fp = final_per_class[cls]["FP"]
    fn = final_per_class[cls]["FN"]
    final_per_class[cls]["precision"] = tp / (tp + fp) if (tp + fp) > 0 else 0.0
    final_per_class[cls]["recall"] = tp / (tp + fn) if (tp + fn) > 0 else 0.0
...
print("\n=== Per-Class Metrics ===")
for cls in sorted(final_per_class.keys(), key=int):
    print(f"Class {cls}: {json.dumps(final_per_class[cls], indent=2)}")
...
with open(output_txt_path, "w") as f:
    ...
    for cls in sorted(final_per_class.keys(), key=int):
        f.write(f"Class {cls}:\n")
        json.dump(final_per_class[cls], f, indent=2)
        f.write("\n")
