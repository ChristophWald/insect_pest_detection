...
def evaluate_from_txt(save_images=False, save_results=True, skip_FRANOC=True):
    import time, os, json, torch, cv2
    start = time.time()
    results = []

    print(f"Evaluating predictions from image segmented labels")

    base_output_path = f"/user/christoph.wald/u15287/insect_pest_detection/2_4_image_processing"
    os.makedirs(base_output_path, exist_ok=True)

    #images are rotated, so original test set images&labels would be wrong
    base_image_path = "/user/christoph.wald/u15287/big-scratch/02_splitted_data/train_labeled/SSL/04_images_cropped"
    base_label_path = "/user/christoph.wald/u15287/big-scratch/02_splitted_data/train_labeled/SSL/04_labels_cropped"
    base_pred_path = "/user/christoph.wald/u15287/big-scratch/02_splitted_data/train_labeled/SSL/05_created_labels_from_test4"

    #only the images of the test set that could be segmented are considered
    filenames = os.listdir("/user/christoph.wald/u15287/big-scratch/02_splitted_data/train_labeled/SSL/03_images_masked")
    filenames.sort()
    ...
    for filename in filenames:
        ...
        tp, fp, fn = compare_labels_vectorized(
            ...
            tile_size=640, iou_threshold=0.5, containment_threshold=0.5, convert_to_xyxy=True
        )
        #print(tp)
...
evaluate_from_txt(save_images=True)